...
description = ''' Ask Steven Shi '''
intents = discord.Intents.all()
intents.members = True

bot = commands.Bot(command_prefix="p!", description=description, intents=intents) # discord bot initialization
...
async def info(ctx, arg, *options):
    ...
    embed = discord.Embed(title=name.upper(), color=discord.Color.from_rgb(204, 0, 0)) # add title and color
    ...
    embed.set_footer(text=f'{name.upper()}', icon_url=pokeball_url)# create a footer with pokeball and name
    embed.set_thumbnail(url=sprite) # set a thumbnail for our embed

    # step 4: send embed to server
    await ctx.send(embed=embed) 

bot.run("Insert Secret Here")